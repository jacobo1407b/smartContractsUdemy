...
export class App extends Component {
    ...
    async componentDidMount() {
        ...
        flightPurchased.watch(function(err,result){
            ...
            if(customer == this.state.account){
                this.container.success(`last customer purchased a flight to ${flight} with a cost of ${price}`,'information we');
            }else{
                this.container.success(`last customer purchased a flight to ${flight} with a cost of ${price}`,'information we');
            }
        }.bind(this));
    ...
    async buyFlight(flightIndex, flight) {
        await this.airlineService.buyFlights(
            flightIndex,
            this.state.account,
            flight.price
        );
            ...
            <div className="row">
                <div className="col-sm">
                    <Panel title="Balance">
                        ...
                        <apan>Balance: {this.state.balance}</apan>
                    </Panel>
                </div>
                <div className="col-sm">
                    <Panel title="Loyalty points - refundable ether">
                        <span>{this.state.refundableEther} eth</span>
                        <button className="btn btn-sm bg-success text-white" onClick={this.refund.bind(this)}>Refund</button>
                    </Panel>
                </div>
            </div>
            <div className="row">
